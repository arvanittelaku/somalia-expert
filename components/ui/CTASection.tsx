...
export function CTASection({
  ...
  description = "Submit your case details for a confidential response within one business day. Legal Aid compatible rates available for all major Somali asylum profiles.",
}: {
  title?: string;
  description?: string;
}) {
    ...
    <section className="bg-[#C8922A] py-12 sm:py-14 md:py-16">
      <div className="mx-auto min-w-0 max-w-6xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <div className="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
          <Link
            href="/contact"
            className="inline-flex min-h-[44px] w-full items-center justify-center rounded-[8px] bg-[#0B2D4E] px-6 py-3 font-semibold text-white transition hover:bg-[#082340] sm:w-auto sm:min-w-[200px] sm:px-8"
          >
            Instruct an Expert
          </Link>
          <Link
